...
class Minecraft(commands.Cog):
    ...
    async def server_action(self, interaction: discord.Interaction):
        ...
        view.add_item(ServerSelectDropdown(self.server_info))
        ...
        for server in servers_in_status:
            ...
            if ":" in address:
                ...
                if port in ("25565", "19132"):
                    address = host  # Не показывать стандартный порт
...
class ServerSelectDropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        server_data = self.view.cog.server_info[channel_id]
        
        modal = ServerSettingsModal(server_data, channel_id)
        await interaction.response.send_modal(modal)

class ServerSettingsModal(discord.ui.Modal):
    def __init__(self, server_data, channel_id):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Сохраняем изменения
        self.view.cog.server_info[self.channel_id] = self.server_data
        ...
        # Обновляем embed сервера
        channel = interaction.guild.get_channel(self.channel_id)
        if channel:
            await self.view.cog.update_server_embed(channel)
...
class ServerSettingsView(discord.ui.View):
    def __init__(self, cog, server_data, channel_id):
        ...
        # Добавляем кнопки для различных настроек
